  ...
  template {
    spec {
      containers {
        ...
        env {
          name  = "POSTGRESQL_URL"
          value = "postgresql://${google_sql_user.app_database_user.name}:${google_sql_user.app_database_user.password}@amplication-staging.csf5nwabym5c.us-east-1.rds.amazonaws.com:5432/app_database"
        }
        env {
